...
class TestSchema (unittest.TestCase) :
    ...
    def test_mano_from_string_class_method_returns_correct_instance (self) :
        ...
        string = ",".join([str(c) for c in cartas])
        mano2 = Mano.from_str(string)
        
        self.assertEqual(mano1, mano2)
        
        self.assertRaises(ValueError, Mano.from_str, string="invalid str")

    def test_mano_class_cannot_have_two_equal_cards (self) :
        self.assertRaises(pydantic.ValidationError, Mano, cartas=[
            Carta(palo=Palo.OROS, numero=12),
            Carta(palo=Palo.OROS, numero=12)
        ])
